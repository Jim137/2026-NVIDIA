...
class MLP(nn.Module):
    ...
    def __init__(self, embed_dim: int, expansion_factor: int = 4):
        ...
        latent_dim = int(math.ceil(math.log2(hidden_dim)))
        self.fc1 = nn.Linear(embed_dim, latent_dim, bias=False)
        self.act = QKAN([latent_dim, latent_dim], reps=1, ba_trainable=True)
        self.fc2 = nn.Linear(latent_dim, embed_dim, bias=False)
